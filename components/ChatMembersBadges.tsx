"use client"

import useAdminId from "@/hooks/useAdminId"
import { chatMembersRef } from "@/lib/converters/ChatMembers"
import { ChatMembers } from "@/lib/types"
import { useCollectionData } from "react-firebase-hooks/firestore"
import LoadingSpinner from "./LoadingSpinner"
import { Badge } from "./ui/badge"
import UserAvatar from "./UserAvatar"


function ChatMembersBadges({chatId} : {chatId:string}) {
  const [members, loading, error] = useCollectionData<ChatMembers>(
    chatMembersRef(chatId))
  const adminId = useAdminId({chatId})


  if (loading && !members) return <LoadingSpinner size={40}/>

  return (
    !loading && (
      <div className="p-2 rounded-xl m-5 bg-contessa-800 shadow-md">
        <div className="flex flex-wrap justify-center md:justify-start items-center gap-2 p-2">
          {members?.map((member) => (
            <Badge
              variant={"secondary"}
              key={member.userId}
              className="h-14 p-5 pl-2 pr-5 flex space-x-2">
              <div>
                <UserAvatar name={member.email} image={member.image} />
              </div>

              <div>
                <p>
                  {member.email}
                </p>
                {member.userId === adminId && (
                  <p className="animated-pulse text-contessa-700 font-bold">Admin</p>
                )}
              </div>
            </Badge>
          ))}
        </div>
      </div>
    )
  )
}

export default ChatMembersBadges